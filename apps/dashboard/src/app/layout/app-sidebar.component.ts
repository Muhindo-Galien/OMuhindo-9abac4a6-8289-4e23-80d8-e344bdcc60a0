import { Component, inject, OnInit, input, output, signal, HostListener, ElementRef } from '@angular/core';
...
@Component({
  ...
  template: `
    ...
    >
      <nav class="p-3 sticky top-14 h-[calc(100vh-3.5rem)] overflow-y-auto flex flex-col">
        <!-- Spaces header: title + + and ... (more) -->
        <div class="border-b border-gray-100">
          <div class="flex items-center justify-between gap-2 px-3 py-2">
            <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Spaces</span>
          ...
          @if (spacesMenuOpen()) {
            <div class="border-t border-gray-100 bg-gray-50/80 py-1" role="menu">
              ...
              @if (canCreateChild()) {
                <button
                  type="button"
                  (click)="createClick.emit(); spacesMenuOpen.set(false); close.emit()"
        ...
        <div class="py-2 flex-1 min-h-0">
          <p class="px-3 py-1.5 text-xs font-medium text-gray-500 uppercase tracking-wider">Recent</p>
          @if (loading()) {
